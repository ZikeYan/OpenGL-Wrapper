...
bool kUnitVisual = false; // pixel unit 1280x960
...
int main() {
  ...
  args1.BindBuffer(2, {GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int),
                       1, GL_UNSIGNED_INT},
                   model.indices().size(), model.indices().data());
  gl::FrameBuffer fbo_image(GL_RGBA, kWindowWidth, kWindowHeight, kUnitVisual);
  ...
  do {
    ...
    if (window.get_key(GLFW_KEY_ENTER)) {
      EncodePixelToUV("beethoven_projection_map.txt", fbo_uv.Capture());
      cv::imwrite("beethoven_screenshot.png", fbo_image.Capture());
    }
